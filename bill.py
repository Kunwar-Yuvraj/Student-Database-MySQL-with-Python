...
def getItemLists():
    ...
    for row in data:
        updateTV.insert('','end',text=row['nameit'],values=(row['name'],row['rate'],row['type'],row['storetype']))
...
def readAllData():
    ...
    for row in data:
        count+=1
        options.append(row['nameit'])
        rateDict[row['nameit']]=row['rate']
...
def addItem():
    ...
    cursor = conn.cursor()
    query = "insert into itemlist (name,nameit,rate,type,storetype) value('{}','{}','{}','{}','{}')".format(name, nameId, rate,Type,storeType)
    cursor.execute(query)
    conn.commit()
    conn.close()
    addItemNameVar.set("")
...
def updateItem():
    ...
    cursor = conn.cursor()
    query = "update  itemlist set name='{}',rate='{}',type='{}',storetype='{}' where nameit='{}'".format(name,rate,Type,storeType,updateItemId)
    cursor.execute(query)
    conn.commit()
    conn.close()
...
def loginWindow():
    titleLabel = Label(window,text="Maharaj Billing System by Jayant",font="Arial 40",fg="green")
